...
class OAuthService {
  constructor(private client: ReturnType<typeof axios.create>) {
    logger.info("oauth-initialized", { baseURL: ENV.oAuthServerUrl ?? null });
    if (!ENV.oAuthServerUrl) {
      logger.error(
        "oauth-server-url-not-configured",
        undefined,
        { hint: "Set OAUTH_SERVER_URL environment variable." }
      );
    }
  }
  ...
  async verifySession(
    ...
    try {
      ...
      if (
        ...
        logger.warn("auth-session-payload-missing-fields");
        return null;
      }
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    if (!user) {
      try {
        ...
        logger.error("auth-oauth-user-sync-failed", error);
