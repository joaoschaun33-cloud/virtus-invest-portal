...
describe("market provider configuration", () => {
  it("reports the availability of all provider credentials", () => {
    const status = getProviderStatus();
    expect(status).toEqual(
      expect.objectContaining({
        brapi: expect.any(Boolean),
        twelveData: expect.any(Boolean),
  ...
  it("documents the source-of-truth coverage without claiming unsupported editorial feeds", () => {
    expect(PROVIDER_COVERAGE.brapi).toEqual({
      quotes: true,
      history: true,
      fundamentals: true,
      news: false,
    ...
    expect(PROVIDER_COVERAGE["twelve-data"]).toEqual({
      quotes: true,
      history: true,
      fundamentals: true,
      news: true,
    ...
    expect(PROVIDER_COVERAGE.finnhub).toEqual({
      quotes: true,
      history: true,
      fundamentals: true,
      news: true,
    ...
    expect(PROVIDER_COVERAGE.coingecko).toEqual({
      quotes: true,
      history: true,
      fundamentals: false,
      news: false,
